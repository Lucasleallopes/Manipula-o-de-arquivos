...
public class Manipulacao_MegaSena {
		...
		try {
			...
			if (NumerosUsuario.length != 6) {
				...
				if (entradaValida) {
					...
					// Os valores na planilha estão em centavos, portanto, dividimos por 100 para convertê-los para reais.
					
					DecimalFormat decimalFormat = new DecimalFormat("#.##");
					System.out.println("|Quantidade de concursos sem ganhador das 6 dezenas: " + semGanhadores + "\n");
					System.out.println("|Menor valor pago para apostas com 4 dezenas: " + decimalFormat.format(minimoPago_4num / 100));
					System.out.println("|Menor valor pago para apostas com 5 dezenas: " + decimalFormat.format(minimoPago_5num / 100));
					System.out.println("|Menor valor pago para apostas com 6 dezenas: " + decimalFormat.format(minimoPago_6num / 100) + "\n");
					System.out.println("|Maior valor pago para apostas com 4 dezenas: " + decimalFormat.format(maximoPago_4num / 100));
					System.out.println("|Maior valor pago para apostas com 5 dezenas: " + decimalFormat.format(maximoPago_5num / 100));
					System.out.println("|Maior valor pago para apostas com 6 dezenas: " + decimalFormat.format(maximoPago_6num / 100) + "\n");
	...
	private static void atualizarFrequencia(Map<Integer, Integer> frequenciaNumeros, int numero) {
	...
	private static int getCellValorInt(Cell cell) {
		if (cell != null) {
	...
	private static double getCellValorDouble(Cell cell) {
		if (cell != null) {
			if (cell.getCellType() == CellType.NUMERIC) {
				return cell.getNumericCellValue();
			} else if (cell.getCellType() == CellType.STRING) {
				String cellValue = cell.getStringCellValue().replaceAll("[^\\d.,]", "");
				cellValue = cellValue.replace(",", ""); // Remova as vírgulas
				cellValue = cellValue.replace(".", ""); // Remova pontos
				cellValue = cellValue.replace(",", "."); // Substitua vírgulas por pontos para tratar decimais
	...
	private static boolean Verificador(int[] NumerosUsuario, int[] NumerosChecar) {
		return Arrays.equals(NumerosUsuario, NumerosChecar);
	}

	private static int[] gerarSorteioAleatorio() {
